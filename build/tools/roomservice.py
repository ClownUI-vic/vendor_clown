...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from ClownUI-Devices Github (http://github.com/ClownUI-Devices)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:ClownUI-Devices+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        repo_remote = repository.get("remote", "clown-devices")
        repo_revision = repository.get('revision')

        if repo_remote != "clown-devices" and not repo_remote.startswith("aosp-"):
        ...
        if repo_revision and not (repo_remote == "clown-devices" and repo_revision == get_default_or_fallback_revision(repo_name)):
...
def get_default_or_fallback_revision(repo_name):
    default_revision = get_remote_revision_from_snippet(".repo/manifests/snippets/clown.xml", "clown-devices")
...
print("Repository for %s not found in the ClownUI-Devices Github repository list. If this is an error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
